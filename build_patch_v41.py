...
assert len(MENU) == 0x842C - 0x83F8  # v33: superseded by MENU_V33 in v41_gates (kept for reference)
...
from v41_gates import V33_SITE_PATCHES, V33_BLOBS, V33_HOOK_PATCHES
...
def main():
    ...
    # v41: star cursor -- copy the star projectile pattern (bank0 sprite tile
    # $02) over bank0 BG-table tile $01.
    # History: v35 used $C1 ('A' in the dark sign font -- corrupted "OCEAN"/
    # "CASTLE"); v39 used $2A, chosen by auditing the five bank-0 RLE screens
    # -- but gameplay rooms ALSO run on bank 0's BG table, drawn from a global
    # 2x2-metatile system (palette table $C279, tile quads $C379 + idx*4,
    # 256 entries), and metatile $01 = quad 29 2A 5B 3A is the static
    # chandelier: the star leaked into it.
    # $01 is the only tile in NO metatile quad (the 256 quads reference 250
    # distinct tile IDs), none of the five RLE screens, no PPU dump, and no
    # dynamic-text range ($30-$39, $41-$5A). The lone LDA #$01 -> $0200,X at
    # $8374 is a packet COUNT byte, not a tile. Vanilla $01 is blank and
    # unreferenced. Since the metatile table serves all four CHR banks, this
    # exclusion over-covers every room on every bank.
    _chr = 0x10 + 0x8000
    _star = bytes(rom[_chr + 0x02*16 : _chr + 0x02*16 + 16])
    _dst = _chr + 0x1000 + 0x01*16
    rom[_dst:_dst+16] = _star; diffs.append((_dst, _star))
    qt = rom[0x10 + 0xC379 - 0x8000 : 0x10 + 0xC379 - 0x8000 + 4*256]
    assert 0x01 not in qt, "tile $01 entered the metatile table -- cursor tile no longer safe"
    print("OK  CHR bank0 $1000 tile $01 <- star projectile pattern (metatile guard passed)")
    ...
    open("mousecapade_2p_v41.nes","wb").write(rom)
    open("mousecapade_2p_v41.ips","wb").write(make_ips(diffs))
    print("Wrote mousecapade_2p_v41.nes and mousecapade_2p_v41.ips")
